...
// Solo para POR_DEVOLVER en ANTICIPO o VIATICOS
if ($estado !== "POR_DEVOLVER" || ($tipo !== "ADELANTO" && $tipo !== "VIATICOS")) {
    echo json_encode(["success" => false, "message" => "No se puede confirmar devolución en este estado"]);
    exit;
}

// Validar archivo
if (!isset($_FILES["comprobante"]) || $_FILES["comprobante"]["error"] !== UPLOAD_ERR_OK) {
    echo json_encode(["success" => false, "message" => "Debe subir un comprobante de devolución"]);
    exit;
}
...
// Historial
$descripcion = "Tesorería confirmó devolución. Comprobante: " . $file["name"];
$hist = $conn->prepare("INSERT INTO solicitud_historial (solicitud_id, usuario_id, accion, descripcion) VALUES (?, ?, 'CONFIRMAR_DEVOLUCION', ?)");
$hist->bind_param("iis", $solicitud_id, $usuario_id, $descripcion);
$hist->execute();

echo json_encode(["success" => true, "message" => "Devolución confirmada correctamente", "estado" => "CERRADO"]);
$conn->close();
?>