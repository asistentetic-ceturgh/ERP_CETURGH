...
const Movilidad = ({ user }) => {
    ...
    const generarPDF = async (movilidad) => {
        ...
        const filas = (movilidad.detalles || []).map(d => ([
            ...
            {
                content: `S/ ${Number(d.monto).toFixed(2)}`,
                styles: {
                    fontStyle: 'bold',
                    halign: 'right'
                }
        ...
        const total = (movilidad.detalles || [])
            .reduce(
        ...
        try {
            ...
            if (movilidad.firma_creador) {

                firmaSolicitante = await getBase64FromUrl(
                    API_BASE + movilidad.firma_creador
                );
            }

            if (movilidad.firma_aprobador) {

                firmaAprobador = await getBase64FromUrl(
                    API_BASE + movilidad.firma_aprobador
                );
            }
        ...
        if (firmaSolicitante) {
            ...
            try {
                ...
                console.error("Firma solicitante error");
            }
        }
        ...
        if (firmaAprobador) {
            ...
            try {
                ...
                console.error("Firma aprobador error");
            }
        }
        ...
        doc.text(
            movilidad.usuario_nombre || "SOLICITANTE",
            55,
            firmaY + 5,
            { align: "center" }
        );
        ...
        doc.text(
            movilidad.aprobador_nombre || "ADMINISTRACIÓN",
            155,
            firmaY + 5,
            { align: "center" }
        );
        ...
        doc.text(
            "Documento generado digitalmente - Copia interna",
            105,
            285,
            { align: "center" }
        );
        ...
        // ===== SAVE =====
        doc.save(
            `Planilla_Movilidad_${movilidad.id}.pdf`
        );
    };
    ...
    // PERMISOS
    // =====================
    const puedeVerTodo = ["TIC", "ADMINISTRACION", "TESORERIA"]
        .includes(currentDept);
    ...
    const loadMovilidad = async () => {
        ...
        try {
            ...
            const normalizado = (Array.isArray(data)
                ...
                ...m,
                detalles: Array.isArray(m.detalles)
                    ? m.detalles
                    : []
            }));
    ...
    // =====================
    const addDetalle = () => {
        setDetalles(prev => [...prev, { fecha: "", monto: "" }]);
    };
    ...
    const guardarEdicionMovilidad = async () => {
        ...
        try {
            ...
            setPlanillaMovilidad(prev =>
                prev.map(m =>
                    m.id === editMovilidadData.id
                        ? { ...m, ...editMovilidadData }
                        : m
                )
            );

            setViewMovilidad(prev =>
                prev
                    ? { ...prev, ...editMovilidadData }
                    : prev
            );
                ...
                prev.map(m =>
                    m.id === id
                        ? {
                            ...m,
                            estado: "Pagado",
                            comprobante_pago: data.archivo,
                            comprobante_tipo:
                                archivoPago.type === "application/pdf"
                                    ? "pdf"
                                    : "imagen"
                        }
                        : m
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                                    Control y seguimiento de viáticos de transporte
                                </p>
                            </div>
                        </div>
                        <button
                            onClick={() => setShowMovilidadModal(true)}
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {getFilteredData().map(m => (
                                        <tr
                                            ...
                                            onClick={() => {
                                                setViewMovilidad(m);
                                                setEditMovilidadData(m);
                                                setDetalles(m.detalles || [{ fecha: "", monto: "" }]);
                                                setEditingMovilidad(false);
                                            }}
                                        ...
                                        >
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-sm font-medium text-slate-700">{m.fecha}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-bold text-[#800000] uppercase tracking-tight">{m.empresa}</span>
                                                    <span className="text-sm text-slate-600 line-clamp-1">{m.motivo}</span>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4">
                                                <span className="text-xs font-bold text-slate-600 uppercase">
                                                    {m.departamento_nombre || m.departamento || `DEPTO #${m.departamento_id}`}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-2 text-slate-600">
                                                    <span className="text-sm bg-slate-100 px-2 py-0.5 rounded text-slate-700 font-medium">{m.origen}</span>
                                                    <ArrowRight size={12} className="text-slate-400" />
                                                    <span className="text-sm bg-slate-100 px-2 py-0.5 rounded text-slate-700 font-medium">{m.destino}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex flex-col items-end">
                                                    <span className="text-sm font-black text-slate-800 tracking-tight">
                                                        S/ {parseFloat(m.monto_total || 0).toFixed(2)}
                                                    </span>
                                                </div>
                                            </td>
                                        </tr>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md transition-all overflow-y-auto">
                    <div className="bg-white w-full max-w-2xl rounded-[2.5rem] shadow-2xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="bg-[#800000] p-6 md:p-8 flex justify-between items-center relative overflow-hidden">
                            ...
                            <button
                                onClick={() => setShowMovilidadModal(false)}
                        ...
                        <form onSubmit={handleCreateMovilidad} className="p-8 md:p-10 space-y-8 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
                                <div className="md:col-span-8">
                                    <label className="block text-[11px] font-black text-[#800000]/60 uppercase tracking-[0.15em] mb-3 ml-1">
                                        Seleccionar Empresa
                                    </label>
                                    <div className="flex flex-wrap gap-3">
                                        <label className="group cursor-pointer">
                                            <input
                                                ...
                                                name="empresa"
                                                value="EDUTUR"
                                                checked={empresa === "EDUTUR"}
                                                className="hidden peer"
                                                onChange={(e) => setEmpresa(e.target.value)}
                                            />
                                            <div className="px-6 py-3 border-2 border-slate-100 rounded-2xl font-bold text-sm peer-checked:border-[#800000] peer-checked:bg-[#800000] peer-checked:text-white peer-checked:shadow-md transition-all group-hover:border-slate-300">
                                                EDUTUR
                                            </div>
                                        </label>
                                        ...
                                        <label className="group cursor-pointer">
                                            <input
                                                ...
                                                name="empresa"
                                                value="KEVSTUR"
                                                checked={empresa === "KEVSTUR"}
                                                className="hidden peer"
                                                onChange={(e) => setEmpresa(e.target.value)}
                                            />
                                            <div className="px-6 py-3 border-2 border-slate-100 rounded-2xl font-bold text-sm peer-checked:border-[#D4AF37] peer-checked:bg-[#D4AF37] peer-checked:text-[#800000] peer-checked:shadow-md transition-all group-hover:border-slate-300">
                                                KEVSTUR
                                            </div>
                                        </label>
                                        ...
                                        {empresa === "EDUTUR" && (
                                            <select
                                                name="sede"
                                                className="px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-sm min-w-[140px] focus:border-[#800000] outline-none transition-all"
                                                defaultValue="CETPRO PIURA"
                                            >
                                ...
                                <div className="md:col-span-4">
                                    ...
                                    <input
                                        ...
                                        type="date"
                                        required
                                        className="w-full p-3 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-sm outline-none focus:border-[#800000] transition-all"
                            ...
                            <div className="space-y-2">
                                <label className="text-[11px] font-black text-[#800000]/60 uppercase tracking-[0.15em] ml-1">
                                    Motivo de la Comisión
                                </label>
                                <input
                                    name="motivo"
                                    required
                                    className="w-full p-4 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-sm focus:bg-white focus:border-[#800000] outline-none transition-all shadow-inner"
                                    placeholder="Ej. Gestión de cobranza externa..."
                                />
                            </div>

                            {/* SECCIÓN 3: TABLA DE DÍAS (MEJORADA) */}
                            <div className="bg-slate-50/50 p-6 rounded-[2rem] border border-slate-100">
                                <div className="flex justify-between items-center mb-4">
                                    <label className="text-[11px] font-black text-[#800000] uppercase tracking-widest ml-2">
                                        Detalle de Gastos por Día
                                    </label>
                                    <button
                                        type="button"
                                        onClick={addDetalle}
                                        className="px-4 py-1.5 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black uppercase tracking-wider hover:bg-blue-100 transition-colors"
                                    >
                                        + Añadir Fila
                                    </button>
                                </div>
                                        ...
                                        <div key={i} className="flex gap-3 items-center group animate-in slide-in-from-left-2 duration-200">
                                            <div className="flex-1 grid grid-cols-2 gap-3">
                                                <input
                                                    ...
                                                    className="p-3 bg-white border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 ring-blue-100 outline-none"
                                                    required
                                                />
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        className="w-full p-3 pl-8 bg-white border border-slate-200 rounded-xl text-sm font-bold focus:ring-2 ring-blue-100 outline-none"
                                                        placeholder="0.00"
                                                        required
                                                    />
                                                </div>
                                ...
                                <div className="mt-6 pt-4 border-t border-slate-200/60 flex justify-between items-center">
                                    ...
                                    <div className="px-6 py-2 bg-white rounded-2xl border border-[#D4AF37]/30 shadow-sm">
                                        ...
                                        <span className="text-xl font-black text-[#800000]">
                                            S/ {detalles.reduce((acc, d) => acc + Number(d.monto || 0), 0).toFixed(2)}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        name="origen"
                                        required
                                        className="w-full p-3 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-sm focus:border-[#800000] outline-none transition-all"
                                ...
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        required
                                        className="w-full p-3 bg-slate-50 border-2 border-slate-100 rounded-2xl font-bold text-sm focus:border-[#800000] outline-none transition-all"
                                        placeholder="Ej. Punto de cobranza"
                                    />
                                </div>
                            </div>

                            {/* FOOTER: Botones con mayor peso visual */}
                            <div className="pt-4 flex gap-4">
                                ...
                                <button
                                    type="submit"
                                    disabled={saving}
                                    className="flex-[2] bg-[#800000] py-4 rounded-2xl text-white text-sm font-black hover:bg-[#600000] hover:shadow-lg hover:shadow-red-900/20 active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50"
                                >
                                    {saving ? (
                                        ...
                                        <><Save size={20} /> REGISTRAR PLANILLA</>
                                    )}
                                </button>
                            </div>
                        </form>
            ...
            {viewMovilidad && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center z-50 p-4 md:p-8 animate-in fade-in duration-500">
                    <div className="bg-white rounded-[3rem] w-full max-w-5xl shadow-[0_40px_80px_-15px_rgba(0,0,0,0.5)] flex flex-col overflow-hidden max-h-[94vh] border border-white/20">
                        ...
                        {/* HEADER: Gradiente Institucional refinado */}
                        <div className="bg-gradient-to-r from-[#800000] via-[#600000] to-[#400000] p-6 md:p-8 text-white flex justify-between items-center relative overflow-hidden">
                            ...
                            <div className="flex items-center gap-6 z-10">
                                <div className="bg-amber-400 p-3.5 rounded-[1.2rem] shadow-lg transform -rotate-3">
                                    <Truck size={28} className="text-[#800000]" />
                                </div>
                                <div>
                                    <div className="flex items-center gap-4">
                                        <h3 className="text-2xl md:text-3xl font-black tracking-tighter uppercase italic">
                                            MOV-{viewMovilidad.id}
                                        </h3>
                                        <BadgeEstado estado={viewMovilidad.estado} />
                                    </div>
                                    <p className="text-[10px] font-bold text-amber-200/60 uppercase tracking-[0.3em] mt-1">
                                        Comprobante Electrónico de Movilidad
                                    </p>
                                </div>
                            </div>

                            <button
                                onClick={() => setViewMovilidad(null)}
                                className="p-3 bg-white/10 hover:bg-white/20 rounded-full transition-all active:scale-90 z-10"
                            >
                                <X size={24} className="text-white" />
                            </button>
                        </div>

                        {/* BODY: Scrollable con secciones claras */}
                        <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 bg-slate-50/50 custom-scrollbar">

                            {/* KPI CARDS: Datos de cabecera */}
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                                {[
                                    ...
                                    <div key={i} className="bg-white p-5 rounded-[2rem] border border-slate-200/60 shadow-sm flex flex-col gap-3 hover:shadow-md transition-shadow">
                                        <div className={`w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center ${info.color}`}>
                                            <info.icon size={20} />
                                        </div>
                                        <div>
                                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{info.label}</p>
                                            <p className="text-base font-black text-slate-800 leading-tight">{info.val}</p>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase mt-1">{info.sub}</p>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            {!editingMovilidad ? (
                                <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    ...
                                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm relative overflow-hidden">
                                        ...
                                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Concepto Detallado</p>
                                        <p className="text-xl font-bold text-slate-800 leading-relaxed italic">
                                            "{viewMovilidad.motivo}"
                                        </p>
                                    </div>

                                    {/* RUTA DINÁMICA */}
                                    <div className="flex flex-col md:flex-row items-center gap-4 bg-white p-2 rounded-[2.5rem] border border-slate-200 shadow-sm">
                                        <div className="flex-1 p-6 text-center md:text-left">
                                            <p className="text-[10px] text-slate-400 uppercase font-black mb-1">Origen</p>
                                            <p className="font-bold text-slate-700 text-lg">{viewMovilidad.origen}</p>
                                        </div>
                                        <div className="bg-slate-100 p-3 rounded-full text-slate-400 transform rotate-90 md:rotate-0">
                                            <ArrowRight size={20} />
                                        </div>
                                        <div className="flex-1 p-6 text-center md:text-right">
                                            <p className="text-[10px] text-slate-400 uppercase font-black mb-1">Destino</p>
                                            <p className="font-bold text-slate-700 text-lg">{viewMovilidad.destino}</p>
                                        </div>
                                    </div>

                                    {/* DETALLE DE GASTOS (SOLO VISTA) */}
                                    <div className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm">
                                        ...
                                        <div className="flex justify-between items-center mb-4">
                                            ...
                                            <span className="text-xs font-bold text-slate-400">
                                                {viewMovilidad.detalles?.length || 0} registros
                                            </span>
                                        </div>

                                        <div className="space-y-3 max-h-52 overflow-y-auto pr-2 custom-scrollbar">
                                            {(viewMovilidad.detalles || []).map((d, i) => (
                                                <div key={i} className="flex justify-between items-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                                                    ...
                                                    <div className="text-right">
                                                        ...
                                                        <span className="text-sm font-black text-[#800000] block">
                                ...
                                <div className="bg-amber-50/50 p-8 rounded-[2.5rem] border-2 border-amber-200 space-y-6 animate-in zoom-in-95 duration-300">
                                    <div className="flex items-center gap-3 mb-2">
                                        ...
                                        <h4 className="font-black text-amber-800 text-sm uppercase tracking-tighter">Modo Edición Activado</h4>
                                    </div>

                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <label className="col-span-full">
                                            <span className="text-[10px] font-black text-amber-800/60 uppercase ml-4">Motivo del Gasto</span>
                                            <input
                                                value={editMovilidadData.motivo}
                                                onChange={(e) => setEditMovilidadData(prev => ({ ...prev, motivo: e.target.value }))}
                                                className="w-full mt-1 p-4 bg-white border-2 border-amber-100 rounded-2xl outline-none font-bold focus:border-amber-400 transition-all shadow-sm"
                                            />
                                        </label>
                                        <label>
                                            <span className="text-[10px] font-black text-amber-800/60 uppercase ml-4">Punto de Origen</span>
                                            <input
                                                value={editMovilidadData.origen}
                                                onChange={(e) => setEditMovilidadData(prev => ({ ...prev, origen: e.target.value }))}
                                                className="w-full mt-1 p-4 bg-white border-2 border-amber-100 rounded-2xl font-bold focus:border-amber-400 outline-none shadow-sm"
                                            />
                                        </label>
                                        <label>
                                            <span className="text-[10px] font-black text-amber-800/60 uppercase ml-4">Punto de Destino</span>
                                            <input
                                                value={editMovilidadData.destino}
                                                onChange={(e) => setEditMovilidadData(prev => ({ ...prev, destino: e.target.value }))}
                                                className="w-full mt-1 p-4 bg-white border-2 border-amber-100 rounded-2xl font-bold focus:border-amber-400 outline-none shadow-sm"
                                            />
                                        </label>
                                        <div className="col-span-full bg-white p-6 rounded-[2rem] border border-amber-200 shadow-inner">
                                            ...
                                            <div className="flex justify-between items-center mb-4">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() =>
                                                        setEditMovilidadData(prev => ({
                                                            ...prev,
                                                            detalles: [...(prev.detalles || []), { fecha: '', monto: '' }]
                                                        }))
                                                    }
                                                    ...
                                                    <div key={i} className="flex gap-3 items-center">
                                                        ...
                                                        <div className="flex-1 grid grid-cols-2 gap-3">
                                                            <input
                                                                ...
                                                                className="p-3 bg-white border border-amber-200 rounded-xl text-sm font-bold outline-none focus:ring-2 ring-amber-200"
                                                                required
                                                            />
                                                            ...
                                                            <div className="relative">
                                                                ...
                                                                <input
                                                                    ...
                                                                    className="w-full p-3 pl-8 bg-white border border-amber-200 rounded-xl text-sm font-bold outline-none focus:ring-2 ring-amber-200"
                                                                    placeholder="0.00"
                                                                    required
                                                                />
                                                            </div>
                                    ...
                                    <div className="flex gap-4 pt-4">
                                        <button
                                            onClick={guardarEdicionMovilidad}
                                            className="flex-1 bg-emerald-600 hover:bg-emerald-700 text-white py-4 rounded-2xl font-black text-sm shadow-lg shadow-emerald-900/20 transition-all active:scale-95"
                                        >
                                            GUARDAR CAMBIOS
                                        </button>
                                        <button
                                            onClick={() => setEditingMovilidad(false)}
                                            className="px-8 py-4 bg-white text-slate-500 border border-slate-200 rounded-2xl font-black text-sm hover:bg-slate-100 transition-all"
                                        >
                                            CANCELAR
                                        </button>
                                    </div>
                            ...
                            {/* SECCIÓN DE FIRMA: Estilo de validación */}
                            <div className={`p-8 rounded-[2.5rem] border-2 border-dashed transition-all
                    ${viewMovilidad.firmado_por
                                ...
                                ${viewMovilidad.firmado_por ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-200 text-slate-400'}`}>
                                            <CheckCircle2 size={32} />
                                        </div>
                                        <div>
                                            <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.2em]">Estado de Autorización</p>
                                            <p className={`text-xl font-black ${viewMovilidad.firmado_por ? 'text-emerald-700' : 'text-slate-500'}`}>
                                                {viewMovilidad.usuario_nombre ? `Validado por: ${viewMovilidad.usuario_nombre}` : 'Esperando firma de Jefatura'}
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    {!viewMovilidad.firmado_por &&
                                        currentRole === "jefe" &&
                                        user?.departamento_id == viewMovilidad.departamento_id && (
                                            <button
                                                onClick={() => firmarMovilidad(viewMovilidad.id)}
                                                className="w-full md:w-auto bg-[#800000] text-white px-10 py-4 rounded-2xl font-black text-sm hover:bg-[#600000] shadow-xl shadow-red-900/20 hover:-translate-y-1 transition-all active:translate-y-0"
                                            >
                            ...
                            {viewMovilidad.comprobante_pago && (
                                <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm">
                                    ...
                                    {viewMovilidad.comprobante_tipo === 'imagen' ? (

                                        <img
                                            src={`${API_BASE}/${viewMovilidad.comprobante_pago}`}
                                            alt="Comprobante"
                                        ...
                                        <a
                                            href={`${API_BASE}/${viewMovilidad.comprobante_pago}`}
                                            target="_blank"
                                            rel="noreferrer"
                                            className="bg-red-600 text-white px-6 py-4 rounded-2xl inline-flex items-center gap-2 font-black"
                                        >
                                            VER PDF
                                        </a>
                        ...
                        {/* FOOTER ACCIONES: Limpio y funcional */}
                        <div className="p-8 bg-white border-t border-slate-100 flex flex-wrap items-center gap-4">

                            {viewMovilidad.estado === "Sin firmar" && !editingMovilidad && (
                                <button
                                    onClick={() => {
                                        setEditingMovilidad(true);
                                        setEditMovilidadData({
                                            ...viewMovilidad,
                                            detalles: Array.isArray(viewMovilidad.detalles)
                                                ? viewMovilidad.detalles
                                                : JSON.parse(viewMovilidad.detalles || '[]')
                                        });
                                    }}
                                    className="bg-slate-900 text-white px-8 py-4 rounded-2xl font-black text-xs hover:bg-slate-800 transition-all flex items-center gap-2 shadow-lg"
                                >
                                    <Edit3 size={16} /> MODIFICAR REGISTRO
                                </button>
                            )}

                            {currentDept === 'ADMINISTRACION' && viewMovilidad.estado === 'Pendiente' && (
                                <div className="flex gap-3">
                                    <button
                                        onClick={() => aprobarMovilidad(viewMovilidad.id)}
                                        className="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black text-xs hover:bg-emerald-700 shadow-lg shadow-emerald-900/20 transition-all"
                                    >
                                        APROBAR
                                    </button>
                                    <button
                                        onClick={() => denegarMovilidad(viewMovilidad.id)}
                                        className="bg-red-50 text-red-600 border border-red-200 px-8 py-4 rounded-2xl font-black text-xs hover:bg-red-100 transition-all"
                                    >
                                        RECHAZAR
                                    </button>
                                </div>
                            ...
                            {currentDept === 'TESORERIA' && viewMovilidad.estado === 'Aprobado' && (
                                <button
                                    onClick={() => setShowPagoModal(true)}
                                    className="bg-[#D4AF37] text-[#800000] px-10 py-4 rounded-2xl font-black text-xs hover:shadow-lg shadow-amber-900/20 transition-all flex items-center gap-2"
                                >
                                    <Wallet size={16} /> EJECUTAR PAGO REEMBOLSABLE
                                </button>
                            )}

                            <button
                                onClick={() => generarPDF(viewMovilidad)}
                                className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-xs hover:bg-blue-700 transition-all"
                            >
                                DESCARGAR PDF
                            </button>
                            ...
                            <button
                                onClick={() => setViewMovilidad(null)}
                                className="ml-auto px-8 py-4 text-xs font-black text-slate-400 hover:text-slate-800 transition-colors uppercase tracking-widest"
                            >
                                Cerrar Vista
                            </button>
                        </div>
                        ...
                        <div className="fixed inset-0 z-[999] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
                            ...
                            <div className="bg-white w-full max-w-lg rounded-[2rem] p-8 shadow-2xl">
                                ...
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-slate-500">
                                            Adjunte el comprobante del depósito
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="mb-5 bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
                                    ...
                                    <p className="text-[10px] uppercase font-black text-emerald-700 mb-1">
                                        Teléfono del solicitante
                                    </p>

                                    <p className="text-2xl font-black text-emerald-800 tracking-wide">
                                        {viewMovilidad.usuario_telefono || 'No registrado'}
                                    </p>

                                    <p className="text-xs text-emerald-600 mt-1">
                                        Útil para Yape o Plin
                                    </p>
                                </div>
                                ...
                                <label className="block">
                                    ...
                                    <input
                                        ...
                                        className="w-full mt-2 border border-slate-200 rounded-2xl p-4"
                                    />
                                </label>

                                {archivoPago && (
                                    <div className="mt-4 bg-slate-100 rounded-xl p-3 text-sm font-bold text-slate-700">
                                        {archivoPago.name}
                                    </div>
                                )}
                                ...
                                <div className="flex gap-3 mt-8">
                                    ...
                                    <button
                                        onClick={() => pagarMovilidad(viewMovilidad.id)}
                                        className="flex-1 bg-[#D4AF37] text-[#800000] py-4 rounded-2xl font-black"
                                    >
                                        CONFIRMAR PAGO
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className="px-6 py-4 rounded-2xl border border-slate-200 font-black text-slate-500"
                                    >
                                        CANCELAR
                                    </button>
